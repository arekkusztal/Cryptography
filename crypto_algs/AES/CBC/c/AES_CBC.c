...
void CBC_encrypt(uint8_t *out, uint8_t *in, uint8_t *key_2,
				enum operation op)
{
	int i;
...
AES_encrypt(uint8_t *out, struct AES_context *ctx, uint16_t block_nr)
{
	...
	for (block = 0; block < block_nr; block ++) {
		...
		CBC_encrypt(out+16*block, key, expansion[0], FIRST);

		for (i = 1; i < 9; i++) {
			CBC_encrypt(out+16*block, key, expansion[i], STD);
		}

		CBC_encrypt(out+16*block, NULL, expansion[9], LAST);
	}
}
